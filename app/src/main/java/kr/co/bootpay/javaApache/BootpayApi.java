...
public class BootpayApi {
    ...
    public void getAccessToken() throws Exception {
        ...
        Log.d("dddddddddddd", "=== token ===" +token);

        Log.d("dddddddddddd", "=== new Gson().toJson(token) ===" +new Gson().toJson(token));

        Log.d("dddddddddddd", "=== new StringEntity ===" +new StringEntity(new Gson().toJson(token), "UTF-8"));

        HttpPost post = getPost(URL_ACCESS_TOKEN, new StringEntity(new Gson().toJson(token), "UTF-8"));
        Log.d("dddddddddddd", "=== post ===" +post);

        HttpClient client = HttpClientBuilder.create().build();
        Log.d("dddddddddddd", "=== client ===" +client);

        HttpResponse res = client.execute(post);
        Log.d("dddddddddddd", "=== res ===" +res);

        String str = IOUtils.toString(res.getEntity().getContent(), "UTF-8");
        Log.d("dddddddddddd", "=== str ===" +str);

        ResToken resToken = new Gson().fromJson(str, ResToken.class);
        Log.d("dddddddddddd", "=== resToken ===" +resToken);

        if(resToken.status == 200)
            this.token = resToken.data.token;
            Log.d("dddddddddddd", "=== resToken.status ==="+resToken.status );
            Log.d("dddddddddddd", "=== this.token ==="+this.token );
    }
